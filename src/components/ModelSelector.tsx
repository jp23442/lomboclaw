...
export function ModelSelector({ models, clientRef }: ModelSelectorProps) {
  ...
  const grouped = useMemo(() => {
    ...
    for (const m of filtered) {
      const provider = getProviderName(m.provider, m.id);
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      >
        {current && <ProviderIcon provider={current.provider} modelId={current.id} size={14} />}
      ...
      {open && (
        <div className="absolute left-0 top-full z-50 mt-1 w-[300px] overflow-hidden rounded-lg border border-zinc-800 bg-[#111111] shadow-xl">
            ...
            {filtered.length === 0 ? (
              ...
              Array.from(grouped.entries()).map(([providerName, providerModels]) => (
                <div key={providerName} className="mb-1">
                  <div className="flex items-center gap-1.5 px-2.5 py-1.5 text-[10px] font-medium uppercase tracking-wider text-zinc-600">
                    <ProviderIcon provider={providerModels[0].provider} modelId={providerModels[0].id} size={12} />
                    {providerName}
                  </div>
                  {providerModels.map((model) => {
                    ...
                    return (
                      <button
                        ...
                        className={`mb-0.5 w-full rounded-md px-2.5 py-1.5 text-left transition ${
                        ...
                        <div className="flex items-center gap-2">
                          <div className="min-w-0 flex-1">
                            ...
                            <div className="flex items-center gap-1.5">
                              <span className="truncate font-mono text-[10px] text-zinc-600">{model.id}</span>
                              {model.reasoning && (
                                <span className="shrink-0 rounded bg-violet-500/15 px-1 py-0.5 text-[8px] font-medium text-violet-300">reasoning</span>
                              )}
                            </div>
                          </div>
                          {selected && (
